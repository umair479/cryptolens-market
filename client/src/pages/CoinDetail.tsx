import { ArrowDownRight, ArrowLeft, ArrowUpRight, BookOpenCheck, CircleAlert, ExternalLink, Globe2, Landmark, RefreshCw, ShieldQuestion } from "lucide-react";
...
export default function CoinDetail() {
  ...
  return <div className="light-app-shell"><MarketHeader active="market" /><main className="data-page coin-detail-page">
    ...
    <section className="coin-detail-hero"><div className="coin-detail-identity"><span className="detail-coin-logo">{coin.image ? <img src={coin.image} alt="" /> : coin.symbol.slice(0, 1)}</span><div><div className="page-overline">RANK #{coin.rank ?? "—"} <span>•</span> LIVE MARKET DATA</div><h1>{coin.name} <span>{coin.symbol}</span></h1><div className="detail-tags">{coin.categories.map((category) => <span key={category}>{category}</span>)}</div></div></div><div className="detail-price"><span>Current price</span><strong>{formatUsd(coin.price)}</strong><Trend value={coin.change24h} /><small>Updated {formatUpdated(coin.lastUpdated ?? undefined)}</small></div></section>
    <section className="coin-detail-grid"><div className="coin-detail-main"><article className="detail-chart-card"><div className="detail-card-head"><div><span className="card-overline">7-DAY PRICE CONTEXT</span><h2>{formatUsd(coin.price)}</h2></div><Trend value={coin.change7d} /></div><DetailSparkline values={coin.sparkline} negative={isNegative} /><div className="detail-chart-caption"><span>7d range • live provider data</span><span>{source.name} • refreshes ~{source.refreshSeconds}s</span></div></article>
      <section className="detail-metrics-grid"><article><span>Market cap</span><strong>{formatCompactUsd(coin.marketCap)}</strong><small>{formatPercent(coin.marketCapChange24h)} 24h</small></article><article><span>24h volume</span><strong>{formatCompactUsd(coin.volume)}</strong><small>Provider reported</small></article><article><span>All-time high</span><strong>{formatUsd(coin.ath)}</strong><small>{formatPercent(coin.athChange)} from ATH</small></article><article><span>All-time low</span><strong>{formatUsd(coin.atl)}</strong><small>Historical provider data</small></article></section>
