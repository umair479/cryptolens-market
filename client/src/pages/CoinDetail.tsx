...
export default function CoinDetail() {
  ...
  return <div className="light-app-shell"><MarketHeader active="market" /><main className="data-page coin-detail-page">
    ...
    <section className="coin-detail-hero"><div className="coin-detail-identity"><span className="detail-coin-logo">{coin.image && !imgError ? <img src={coin.image} alt={coin.name} width={42} height={42} style={{ borderRadius: "50%", objectFit: "contain" }} onError={() => setImgError(true)} /> : coin.symbol.slice(0, 1)}</span><div><div className="page-overline">RANK #{coin.rank ?? "—"} <span>•</span> LIVE MARKET DATA</div><h1>{coin.name} <span>{coin.symbol}</span></h1><div className="detail-tags">{coin.categories.map((category) => <span key={category}>{category}</span>)}</div></div></div><div className="detail-price"><span>Current price</span><strong>{formatUsd(coin.price)}</strong><Trend value={coin.change24h} /><small>Updated {formatUpdated(coin.lastUpdated ?? undefined)}</small></div></section>
