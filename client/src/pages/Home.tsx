...
export default function Home() {
  ...
  function toggleWatchlist(coin: LiveCoin) {
    ...
    if (savedIds.has(coin.id)) removeFromWatchlist.mutate({ coinId: coin.id }); else addToWatchlist.mutate({ coinId: coin.id });
  }
  ...
  return <div className="light-app-shell"><MarketHeader active="market" search={search} onSearchChange={setSearch} watchCount={savedIds.size} /><div className="market-ticker"><div className="ticker-inner"><span><strong>Global provider</strong> {market.isError ? "Unavailable" : data?.sources.global ?? "Connecting"}</span><span><strong>Binance spot</strong> {data?.binance.status === "live" ? "Live" : "Unavailable"}</span><span><strong>Refresh</strong> {data?.sources.refreshSeconds ?? 60}s</span><span className="ticker-updated">{market.isError ? "Provider error" : `Updated ${formatUpdated(data?.overview.updatedAt)}`}</span></div></div><main className="market-page">
    ...
    {market.isError ? <section className="market-error"><LineChart size={22} /><div><strong>Live market data is temporarily unavailable.</strong><p>{market.error.message || "We could not refresh provider data. The page will recover automatically or you can retry now."}</p></div><button className="outline-button" onClick={() => market.refetch()}><RefreshCw size={14} /> Retry</button></section> : <>
      ...
      <section className="table-section" id="live-rankings"><div className="section-heading"><div><div className="page-overline">LIVE RANKINGS <span>•</span> {board.toUpperCase()}</div><h2>{board === "gainers" ? "Top 20 gainers" : board === "decliners" ? "Top 20 decliners" : board === "watchlist" ? "Your saved assets" : categoryId === "all" ? "All cryptocurrencies" : categories.data?.find((category) => category.id === categoryId)?.name ?? "Category assets"}</h2></div><button className="outline-button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>USD <ChevronDown size={14} /></button></div><div className="table-controls"><div className="asset-tabs" role="tablist" aria-label="Market ranking filters">{([ ["all", "All assets"], ["gainers", "Top gainers"], ["decliners", "Top decliners"], ["watchlist", "Saved"] ] as const).map(([value, label]) => <button key={value} className={board === value ? "is-selected" : ""} onClick={() => setBoard(value)} role="tab" aria-selected={board === value}>{label}</button>)}</div><div className="control-actions"><label className="table-search"><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search coins" aria-label="Search coins in market rankings" /></label><button className="filter-button" onClick={() => setSegment((current) => current === "volume" ? "all" : "volume")}><Filter size={15} /> {segment === "volume" ? "Volume" : "Rank"}</button></div></div><div className="market-segments" aria-label="Market segments">{([ ["all", "All market"], ["large", "Top 25"], ["mid", "Ranks 26–100"], ["volume", "Volume leaders"] ] as const).map(([value, label]) => <button key={value} className={segment === value ? "is-selected" : ""} onClick={() => setSegment(value)}>{label}</button>)}</div><div className="category-filter-row" aria-label="Live market categories"><span>Category</span><button className={categoryId === "all" ? "is-selected" : ""} onClick={() => setCategoryId("all")}>All</button>{(categories.data ?? []).slice(0, 7).map((category) => <button key={category.id} className={categoryId === category.id ? "is-selected" : ""} onClick={() => setCategoryId(category.id)}>{category.name}</button>)}</div><div className="ranking-table-wrap"><div className="ranking-table" role="table" aria-label="Live cryptocurrency rankings"><div className="ranking-row ranking-header" role="row"><span>#</span><span>Name</span><span>Price</span><span>24h</span><span>7d chart</span><span>Market cap</span><span>Volume 24h</span><span /></div>{market.isLoading || (categoryId !== "all" && categoryMarket.isLoading) ? Array.from({ length: 12 }).map((_, index) => <div className="ranking-row ranking-loading" key={index}><span /><span /><span /><span /><span /><span /><span /></div>) : displayedCoins.map((coin) => <div className="ranking-row" role="row" key={coin.id}><span className="rank-number">{coin.rank}</span><div className="ranking-asset"><button className={`star-button ${savedIds.has(coin.id) ? "is-saved" : ""}`} onClick={() => toggleWatchlist(coin)} aria-label={`${savedIds.has(coin.id) ? "Remove" : "Add"} ${coin.name} ${savedIds.has(coin.id) ? "from" : "to"} watchlist`} aria-pressed={savedIds.has(coin.id)}><Star size={15} fill={savedIds.has(coin.id) ? "currentColor" : "none"} /></button><CoinGlyph coin={coin} /><Link href={`/coin/${coin.id}`} className="ranking-asset-link"><strong>{coin.name}</strong><span>{coin.symbol}</span></Link></div><strong className="ranking-price">{formatUsd(coin.price)}</strong><ChangeValue value={coin.change24h} /><Sparkline values={coin.sparkline} tone={coin.change24h < 0 ? "negative" : "positive"} /><span className="ranking-value">{formatCompactUsd(coin.marketCap)}</span><span className="ranking-value volume-value">{formatCompactUsd(coin.volume)}</span><Link href={`/coin/${coin.id}`} className="row-menu" aria-label={`Open ${coin.name} details`}><ChevronRight size={16} /></Link></div>)}{!market.isLoading && !(categoryId !== "all" && categoryMarket.isLoading) && !displayedCoins.length && <div className="empty-ranking"><Search size={18} /><strong>No matching live assets</strong><span>Try a new ticker, clear the search, or switch the table filter.</span><button className="clear-button" onClick={() => { setSearch(""); setBoard("all"); setSegment("all"); setCategoryId("all"); }}>Clear filters</button></div>}</div></div><div className="ranking-pagination"><span>Showing {filteredCoins.length ? (page - 1) * PAGE_SIZE + 1 : 0}–{Math.min(page * PAGE_SIZE, filteredCoins.length)} of {filteredCoins.length} assets</span><div><button onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} aria-label="Previous page"><ChevronLeft size={15} /></button>{Array.from({ length: Math.min(totalPages, 4) }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} onClick={() => setPage(pageNumber)} className={page === pageNumber ? "is-current" : ""}>{pageNumber}</button>)}{totalPages > 4 && <span>…</span>}<button onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages} aria-label="Next page"><ChevronRight size={15} /></button></div></div><div className="table-summary"><span><span className="status-dot-light" /> Data through {categoryId === "all" ? data?.sources.global ?? "provider connection" : "CoinGecko category market feed"}</span><span>Page {page} of {totalPages}</span></div></section>
